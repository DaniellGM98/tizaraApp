...
class MyDateUtil {
  ...
  static String getLastActiveTime(
    ...
    if (i == -1) return 'Última visita no disponible';
    ...
    if (time.day == now.day &&
        ...
        time.year == time.year) {
      return 'Visto por última vez hoy a las $formattedTime';
    }

    if ((now.difference(time).inHours / 24).round() == 1) {
      return 'Visto por última vez ayer a las $formattedTime';
    }

    String month = _getMonth(time);

    return 'Visto por última vez ${time.day} $month $formattedTime';
  }
